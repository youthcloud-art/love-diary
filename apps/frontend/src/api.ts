...
export const saveTokens=(accessToken:string,refreshToken:string)=>{uni.setStorageSync("access_token",accessToken);uni.setStorageSync("refresh_token",refreshToken)};
export async function request<T>(url:string,method:"GET"|"POST"|"PATCH"|"DELETE"="GET",data?:any):Promise<T>{
 return new Promise((resolve,reject)=>uni.request({url:base+url,method,data,header:{Authorization:token()?`Bearer ${token()}`:""},success:r=>r.statusCode>=200&&r.statusCode<300?resolve(r.data as T):reject(new Error((r.data as any)?.detail||`请求失败 ${r.statusCode}`)),fail:()=>reject(new Error("无法连接到服务，请确认后端已启动"))}));
}
export async function auth(path:string,data:any){const r=await request<any>(`/api/v1/auth/${path}`,"POST",data);saveTokens(r.access_token,r.refresh_token)}
